...
<?php
$connection = mysqli_connect("localhost", "root", "", "rpims_db");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->query("UPDATE staff SET userpassword='$password' WHERE id='$update';");
...
//supplier update
if (isset($_GET['supplier_update'])) {
    $update = $_GET['supplier_update'];
    ...
    while ($row1 = mysqli_fetch_array($query1)) {
        ...
        echo "<input type='text' name='suppliername' value='{$row1[1]}' disabled/><br/>";
...
if (isset($_GET['purchase_update'])) {
    ...
    while ($row1 = mysqli_fetch_array($query1)) {
        ...
        echo "<input type='number' name='unitprice' value='{$row1[7]}' disabled/><br/>";
        echo "<label>Quantity:</label><br />";
        echo "<input type='number' name='quantity' value='{$row1[8]}' disabled/><br/>";
        echo "<label>Purchase Amount:</label><br />";
        echo "<input type='number' name='purchase_amount' value='{$row1[9]} id='purchase_amount' disabled'/><br/>";
...
if (isset($_GET['debt_update'])) {
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        echo "<input type='text' name='customername' value='{$row['customer_name']}' disabled/><br/>";
        echo "<label>Medicine Name:</label><br />";
...
if (isset($_GET['delete_med'])) {
    $update = $_GET['delete_med'];
    $query = $connection->query("DELETE FROM medicine_info WHERE medicine_id=$update");
    if(!$query){
        echo "Err:".$connection->error;
    }
    else{
